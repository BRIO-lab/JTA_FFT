        ...
        for pat_id in [x for x in os.listdir(study_org_dir) if os.path.isdir(study_org_dir + "/" + x)]:
            ...
            for sess_id in [x for x in os.listdir(pat_dir) if os.path.isdir(pat_dir + "/" + x)]:
                ...
                for mvt_id in [x for x in os.listdir(sess_dir) if (os.path.isdir(sess_dir + "/" + x))]:
                    ...
                    elif os.path.exists(mvt_dir + "/1024/cal1024.txt"):
                        calibration = mvt_dir + "/1024/cal1024.txt"
                    else:
                    ...
                    try:
                        ...
                        print("Error loading calibration files: ", mvt_dir)
                        continue

                    try:
                        FEM_NFD.load_nfd_library(fem_nfd_path)
                        TIB_NFD.load_nfd_library(tib_nfd_path)
                    ...
                    try:
                        for idx, fem_img in enumerate(os.listdir(fem_img_dir)):
                            ...
                            xt,yt,zt,zr,xr,yr = FEM_NFD.pose_from_segmentation(fem_img_dir + "/" +fem_img)
                    ...
                    try:
                        for idx, tib_img in enumerate(os.listdir(tib_img_dir)):
                            ...
                            xt,yt,zt,zr,xr,yr = TIB_NFD.pose_from_segmentation(tib_img_dir + "/" +tib_img)
